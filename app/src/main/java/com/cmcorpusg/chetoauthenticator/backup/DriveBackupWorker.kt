    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (authResult.hasResolution() || authResult.accessToken.isNullOrBlank()) {
                settings.lastError = "Google Drive needs authorization in the app"
                return@withContext Result.retry()
            }

            val plain = SecureAccountStore(applicationContext).exportPlainJson()
            val encrypted = BackupCrypto.encrypt(plain, key, salt)
            DriveBackupClient().upload(authResult.accessToken!!, encrypted)
